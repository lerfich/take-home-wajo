# Run Wajo from a fresh copy

Wajo runs locally on your computer. The reviewer flow is the same with either installation method: start Wajo, connect Gmail through Google, confirm the account and synchronization scope in Wajo, then use the inbox. The checked-in bundled Groq evaluation key is ready without `.env`, a key file, or a Groq account. Internet access is required for Groq inference; the free account can hit its quota. Gmail access requires a Google Desktop OAuth client and separate consent.
...
On first launch, this creates `.venv`, installs the pinned Gmail dependencies, and creates `data/web-groq.sqlite3`. Open <http://127.0.0.1:8765/>. Stop with Ctrl+C. Later launches reuse the same local database. Do not start Docker against the same `task/data` directory at the same time.
