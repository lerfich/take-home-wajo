...
During a brief live startup, the saved Gmail connection was observed to target a different account from the earlier Wajo-Test context. The server was stopped immediately; automatic polling had imported two more incoming messages, leaving **50 local emails**. No send or deletion was performed. Read-only checks still show action #42 pending at revision 1 with no sent row, and action #43 executed from its prior approval. The mailbox and OAuth state were not changed, and the two imported messages remain in the local database. Further live pilot work requires Nikita's explicit account/scope choice; subsequent UI verification used only a separate synthetic demo database.
