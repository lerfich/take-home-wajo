...
- Gmail polling, pagination, period summaries, reminders, Docker and comprehensive final evaluation are not implemented yet. Automatic replies are intentionally excluded: Skills can improve draft wording but never authorize sending. Current Gmail import reads one bounded page and requires Wajo-Test in the UI/live executor. Gmail import, reversible writes and one approved synthetic self-send have been exercised; these are small transport checks, not a final evaluation. Skills use finite semantic categories and have synthetic/mocked verification, not established production quality or a private-mail pilot.
