...
Open [the local app](http://127.0.0.1:8765/). Add a synthetic email; background workers invoke the active model provider and the page refreshes the resulting decisions. Bundled Groq is fixed at three concurrent processors; user Groq and user OpenAI allow 3–12. Queue state, retry timing and provider diagnostics persist in SQLite. Interrupted or failed analysis is retried because an email is not considered processed until its complete decision is stored. Repeated Gmail message IDs do not duplicate actions.
