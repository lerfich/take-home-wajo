...
function independentLabelBlock(row){
  ...
  if(label.status==='awaiting_confirmation')return {required:true,content:`<div class="decision label-decision"><p>Wajo suggests a Gmail label for this received email. Confirm or change it; this is separate from archiving, replies and events.</p><form id="independent-label-form"><label>Suggested Gmail label<input name="label" maxlength="100" required value="${esc(suggested)}" aria-label="Gmail label"></label><small>Only an AI: label is written to Gmail after your confirmation. Topic, subtype and related themes remain in Wajo.</small><div class="decision-buttons"><button class="primary" type="submit">Confirm label</button><button class="secondary reject-button" type="button" id="skip-independent-label">Do not add label</button></div></form></div>`};
  if(['executing','unknown','error'].includes(label.status))return {required:['unknown','error'].includes(label.status),content:`<div class="decision label-decision"><p>${esc(label.error||'Checking whether Gmail saved the confirmed label…')}</p><button class="secondary" id="check-independent-label">Check Gmail status</button></div>`};
  return {required:false,content:`<div class="decision label-decision"><p>${label.status==='skipped'?'No label added after your decision.':`Label ${esc(label.current_label||suggested)} ${label.status==='automatic'?'applied automatically':'confirmed'} in Gmail.`}</p></div>`};
}
...
function renderDetail(row){
  ...
  $('#independent-label-form')?.addEventListener('submit',async e=>{e.preventDefault();const result=await post('/api/label-decision',{action_id:row.id,revision:row.independent_label.revision,choice:'confirm',label:new FormData(e.currentTarget).get('label')});if(result)notify('Checking the confirmed Gmail label.');});
