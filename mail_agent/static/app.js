...
function render(){
  ...
  $('#new-email').classList.add('hidden');$('#load-demo').classList.toggle('hidden',!demo);
  $('#mode-note').textContent=demo?'Sample cases: emails and decisions are predefined; no AI model is called. All mail actions are simulated locally.':state.gmail_enabled?`Gmail live · ${provider}: synchronized incoming mail can receive AI labels, be archived, or have replies saved as Gmail drafts. Sending follows the separate approval and Superpowers policy.`:`Connect Gmail to start syncing. You can change the model provider in Models.`;
...
function decorateDecisionButtons(root=document,context=''){
  root.querySelectorAll('button:is(.primary,.escalation-final):not([disabled]):not(.hidden)').forEach((button,index)=>{
...
function archiveDecisionBlock(row){
  ...
  const explanation=`<p><strong>Mailward recommends: ${recommendation}</strong></p><p>${esc(d.reason||'Based on this email’s meaning and context.')}</p>${d.evidence?`<blockquote>${esc(d.evidence)}</blockquote>`:''}`;
...
function independentLabelBlock(row){
  ...
  if(label.status==='awaiting_confirmation')return {required:true,content:`<div class="decision label-decision"><p>Mailward suggests a Gmail label for this received email. Confirm or change it; this is separate from archiving, replies and events.</p><form id="independent-label-form"><label>Suggested Gmail label<input name="label" maxlength="100" required value="${esc(label.labels?.[0]||suggested)}" aria-label="Gmail label"></label><label>Second Gmail label · optional<input name="second_label" maxlength="100" value="${esc(label.labels?.[1]||'')}" aria-label="Second Gmail label"></label><small>Up to two AI: labels are written to Gmail after your confirmation. Topic, subtype and related themes remain in Mailward.</small><div class="decision-buttons"><button class="primary" type="submit">Confirm label</button><button class="secondary reject-button" type="button" id="skip-independent-label">Do not add label</button></div></form></div>`};
...
function renderDetail(row){
  ...
  const escalation=row.status==='escalated'||row.status==='blocked'?`<div class="human-review requires-review" data-review-key="escalation"><strong>${row.status==='blocked'?'Action blocked':'Your judgment is needed'}</strong><p>${esc(row.reason||p.reason)}</p><small>Mailward has no safe action to approve. Choose how you want to track the situation; neither option executes the request in the email.</small><div class="decision-buttons"><button class="primary escalation-final escalation-purple" data-escalation-choice="handled">I’ll handle this</button><button class="secondary escalation-final escalation-orange" data-escalation-choice="attention">Keep in Needs attention</button></div></div>`:row.status==='reviewed'?'<div class="human-review"><strong>Reviewed by you</strong><p>You chose to handle this outside Mailward. No email action was approved or executed.</p></div>':'';
  ...
  const replyPanel=draft?`<section class="reply-pane ${canEdit?'requires-review':''}"><div class="reply-heading"><strong class="${canEdit?'review-pulse':''}">Draft prepared by Mailward</strong><div class="reply-actions">${canEdit?'<button class="secondary reject-button" id="reject">Reject</button><button class="primary review-pulse" id="approve">Approve and send</button><button class="secondary review-pulse" id="edit-reply">Edit</button>':''}</div></div><div id="reply-preview" class="send-preview"><div class="reply-address"><small>From</small> ${esc(draft.sender)}</div><div class="reply-address"><small>To</small> ${esc(draft.recipient)}</div><div class="reply-address"><small>Subject</small> ${esc(draft.subject)}</div><p>${esc(draft.text)}</p></div>${canEdit?'<div id="reply-editor" class="hidden"><label>From<input id="edit-from" disabled></label><label>To<input id="edit-recipient" disabled></label><label>Subject<input id="edit-subject" maxlength="300"></label><textarea id="edit-text" aria-label="Reply text" rows="5"></textarea></div>':''}${row.status==='rejected'?'<p>The reply was rejected. Its unsent Gmail draft is retained.</p>':''}${draftStyleForm(row)}</section>`:'';
...
function openEvent(id){
  ...
  $('#event-mistake')?.addEventListener('click',async()=>{const result=await post('/api/events/mistake',{event_id:Number(item.id)});if(result){$('#event-dialog').close();notify('Event removed. Mailward will ask again in similar situations.')}});
...
function updateModelControls(){
  ...
  $('#model-quota-copy').textContent=bundled?'The bundled free tier supports about 65 emails per day. Actual capacity varies with message length and Groq limits. Concurrency is fixed at 3.':mode==='user_groq'?'Your Groq account limits and availability apply. Higher concurrency can reach your quota sooner.':'OpenAI API usage is billed to your account. Higher concurrency can increase spend and reach rate limits sooner.';
  $('.model-apply .risky').classList.toggle('hidden',mode!=='user_openai');
  $('#model-risk-copy').textContent=bundled?'Changing provider affects future AI analysis. The bundled option does not use a personal key.':mode==='user_openai'?'OpenAI requests use your paid API key. Review account usage and results after switching.':'Your provider quota applies to future model requests. Keep this key private.';
...
function renderSuperpowers(){
  ...
  $('#open-superpowers').textContent=enabled?'Tune Mailward’s superpowers':'Want to give Mailward superpowers?';
...
const composeKey='mailward-unsent-test-email';
...
function renderGmail(){
  ...
  $('#gmail-intro').textContent=choosing?'Google connected a different Gmail account. Mailward will not synchronize it until you choose what happens to previous local data.':connected?'Your account is connected. You can sync emails below.':g.token_present?'A saved connection is available. Check its status before signing in again.':'Connect your account through Google. Mailward never asks for your Gmail password.';
...
$('#gmail-account-choice-form').addEventListener('submit',async e=>{e.preventDefault();const g=state.gmail_connection;const choice=new FormData(e.target).get('account-choice');if(await gmailPost('/api/gmail/account-choice',{account:g.account,previous_account:g.previous_account,choice}))notify(choice==='fresh'?'Previous local Mailward data removed. Gmail was not changed.':'Previous Mailward data kept in the shared timeline.')});
